...
function RouteManager() {
  ...
  const handleBookingState = (route: string) => {
    ...
    setBooking(bookingRouteState({ route: route, isActive: isON }))
    console.log(booking.map((e) => e.route == route), 'in')
  };
  console.log(booking, 'redux');
  ...
  return (
    <section>
      <div className=" flex flex-col gap-y-5 justify-center items-center bg-gray-200 text-black mx-auto py-24 ">
        <h4 className="text-xl uppercase font-bold md:text-4xl">manage routes</h4>
        <div className="grid sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 py-10 lg:gap-x-10 ">
          {/* BOOKING ROUTE */}
          {
            bookingOption.map((bookingRoute, idx) =>
              <div key={idx} className="mx-4 bg-white p-8 rounded-lg border-b-4 border-blue-900 sm:mb-8 ">
                <div className=" flex space-x-12 items-center justify-between border-b-2 border-gray-400 py-2 pb-4">
