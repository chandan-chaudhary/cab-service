...
const BookingDetails =()=> {
    ...
    const [assignCab, setAssignCab] = useState<string>('');
    const [showBooking,setShowBooking] = useState<boolean>(true);

    const handleAssignCab = (assigningType:string) =>{
        if(assigningType === 'confirmed') return 'text-green-600';
        if(assigningType === 'canceled') return 'text-red-500';
    }
  return (
    <section>
        <div className='bg-gray-100 text-blue-950 px-5 py-12 '>
            ...
            {/* ASSIGN CAB DETAILS */}
            <div className={` ${showBooking ? '': 'hidden'} flex flex-col border-2 border-gray-300 bg-white rounded-lg space-y-6 p-5 my-5`}>
